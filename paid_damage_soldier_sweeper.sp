...
#define ROBOT_DESCRIPTION "Rocket Reserve Shooter"
#define ROBOT_COST 1.5
#define ROBOT_STATS "Shoot down blast jumpers"
#define ROBOT_ON_DEATH "Reflect rocket bursts for massive damage"
...
stock GiveGiantPyro(client)
{
	if (IsValidClient(client))
	{
		...
		CreateRoboWeapon(client, "tf_weapon_shotgun_soldier", 415, 8, 1, 2, 210);
		
		
		CreateRoboHat(client, WarPig, 10, 6, 0.0, 1.0, -1.0);
		CreateRoboHat(client, ManInSlacks, 10, 6, 0.0, 1.0, -1.0);
		CreateRoboHat(client, Coif, 10, 6, 0.0, 1.0, -1.0);
	...
	//	CreateHat(client, 30050, 10, 6, true); //Gatebot
		...
		if(IsValidEntity(Weapon2))
		{
			...
			TF2Attrib_SetByName(Weapon2, "weapon spread bonus", 0.8);
			...
			TF2Attrib_SetByName(Weapon2, "mini rockets", 1.0);
			TF2Attrib_SetByName(Weapon2, "projectile speed increased", 1.8);
			TF2Attrib_SetByName(Weapon2, "dmg penalty vs buildings", 0.25);
			TF2Attrib_SetByName(Weapon2, "dmg penalty vs players", 1.1);
			TF2Attrib_SetByName(Weapon2, "Blast radius increased", 0.5);
